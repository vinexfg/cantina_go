...
function parsePagination(query) {
  ...
  const limit = Math.min(MAX_PAGE_LIMIT, Math.max(1, parseInt(query.limit) || 20));
  return { page, limit };
}

function buildPaginacao(page, limit, total) {
...
class ReservaController {
  ...
  static async obterPorCantina(req, res, next) {
    try {
      ...
      Result.okPaginado(dados, buildPaginacao(page, limit, total)).send(res);
    } catch (erro) {
      next(erro);
    }
  }
